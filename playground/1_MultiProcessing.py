...
# %%
def total_primes(nombres):
    return sum([is_prime(nombre) for nombre in nombres])
# %%
...
def split_total(N):
    premiere_moitie = range(1, N // 2 + 1)
    seconde_moitie = range(N // 2 + 1, N + 1)
    with Pool(2) as pool:
        return sum(pool.map(total_primes,[premiere_moitie, seconde_moitie]))
...
def multi_process_list(f,n,par_f,l):
    partitions = par_f(l, n)
    with Pool(ncores) as pool:
        return sum(pool.map(f, partitions ))
...
def naive_par(lst,n):
    length = len(lst)
    partition_size = length // n
    partition = []
    start = 0
    for i in range(n - 1):
        end = start + partition_size
        partition.append(lst[start:end])
        start = end 
    partition.append(lst[start:])   
    return partition 
