...
val_reference_time = %timeit -o np.mean(get_pools_strides(img=image, pool_size=2, stride=2), axis=(2,3))

val_reference_time.average
...
def strong_scaling_main(img,ncores):
    strides = get_pools_strides(img, pool_size=2, stride=2)
    split = np.array_split(strides, ncores)

    with ThreadPool(ncores) as pool:
        resultat = pool.map(meanpool, split)

    return np.vstack(resultat)
...
print(f"Speedup: {val_reference_time.average / time_threaded.average}")

#%%
def meanpool2(a):
    return np.mean(get_pools_strides(a, pool_size=2, stride=2)
,axis=(2,3))

def strong_scaling_main2(img,ncores):
    split = np.array_split(img, ncores)

    with ThreadPool(ncores) as pool:
        resultat = pool.map(meanpool2, split)
...
N=10000
img=np.arange(N*N).reshape(N,N)
res_thread = strong_scaling_main2(img,ncores=8)
np.testing.assert_array_equal(res_single, res_thread)

time_threaded = %timeit -o strong_scaling_main2(img,ncores=8)
print(f"Speedup: {val_reference_time.average / time_threaded.average}")
...
def weak_scaling_run(compute_func,ncores_range,N,verbose=True):
    ...
    for nbr_parallel_blocks in ncores_range:
        ...
        t2 = time.time()
        compute_func([imgs[0]],1)
        total_time_seq = (time.time() - t2)*nbr_parallel_blocks
        if verbose:
...
# %%
strong_run2 = strong_scaling_run(strong_scaling_main2,range(1,9),10000)
# %% [markdown]
"""
...
# %%
s_strong2 = find_s_amdahl(strong_run2)
compare_law_and_run(strong_run2,amdahl_func,range(1,9),s_strong2,"Strong run 2","Amdahl")
